...
- ALWAYS: Put every test under the repo-root `tests/` directory, mirroring the path of the file it covers (`api/hono/src/lib/imdb.ts` is tested by `tests/api/hono/src/lib/imdb.test.ts`); `bun run test` runs the suite. A test imports by relative path and reaches only pure modules (no env, no database, no extension API), which is why the API keeps its parsing and matching (`lib/imdb.ts`, `lib/lookup.ts`) apart from its querying (`lib/ratings.ts`, `jobs/imdb-sync.ts`), and the extension keeps its DOM work (`utils/netflix.ts`) free of `wxt/*` imports.
...
| [imdb-sync](.agents/skills/imdb-sync/SKILL.md)           | Build, refresh, or debug the IMDb index (imdb_title, imdb_name) that answers every rating.                                                                                          |
