...
// Without this Next serves its own bare document (<html id="__next_error__">), which renders outside the root layout: no theme class, no fonts, so a 404 flashes white before or instead of the app's own styling. This one renders inside the layout, so it is themed like the rest of the app. It catches notFound() from a page, not from a layout that has already begun streaming: that one still reaches Next's internal fallback.
