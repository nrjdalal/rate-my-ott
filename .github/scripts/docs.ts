...
// URL base per docs.config collection; must match its loader baseUrl in source.ts.
const BASE: Record<string, string> = { docs: "/docs" }
...
async function run() {
  ...
  for (const [name, collection] of Object.entries(docsConfig)) {
    ...
    // Content dir mirrors the URL base: /docs -> content/docs (the on-disk path matches the route).
