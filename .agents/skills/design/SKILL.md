...
- Anything painted onto netflix.com lives in `extension/wxt/assets/netflix.css`: every class prefixed `rmo-`, each rule setting only what it needs, no reset and no Tailwind, so nothing restyles the page. The card badge is one label hanging from the top edge of the artwork at its right, 8px in from the corner, flush with the edge and rounded only below (the counterpart of Netflix's own "Recently added" label at the bottom left), numbers only, one partition per platform in its hue (IMDb yellow today; the structure takes more) as a tinted fill with the number in the hue over a dark translucent base, no separators and no border, a missing score dropping its partition; `pointer-events: none` so the card still hovers and clicks. The modal's rating is one more row of Netflix's own details column ("IMDb: 5.4 with 20.2K votes" beside Cast and Genres), built from Netflix's row classes so it reads as Netflix's, with `.rmo-panel` only marking it as ours.
