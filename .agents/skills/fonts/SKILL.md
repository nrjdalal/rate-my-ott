...
- Preload is scoped by import. Any font in `web/next/src/lib/fonts.ts` preloads on every page, because the root layout imports it. To scope a font to specific routes, declare it in a module only those routes import (e.g. `web/next/src/lib/marketing/fonts.ts` with woff2 under `web/next/src/fonts/marketing/`).
