...
- A headed, logged-in Netflix for checks: `AGENT_BROWSER_HEADED=1 AGENT_BROWSER_PROFILE=~/.agent-browser/profiles/rate-my-ott-netflix AGENT_BROWSER_EXTENSIONS=$PWD/extension/wxt/.output/chrome-mv3 agent-browser --session netflix open https://www.netflix.com/browse` (the persistent profile keeps the login; sign in by hand the first time), then `agent-browser eval "document.querySelectorAll('.rmo-badge').length"`. Run the API on the baked port for it: `cd api/hono && PORT=4000 bun src/index.ts`.
