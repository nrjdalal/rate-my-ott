...
export const site = {
  ...
  apiReferenceDescription: `The API behind the Rate My OTT browser extension: ratings lookups answered from the IMDb index, IMDb's daily datasets in Postgres.
...
- \`packages/db/\` - Drizzle ORM schema + client (PostgreSQL via Bun's SQL driver); the \`imdb_title\` and \`imdb_name\` tables are the IMDb index that \`bun run imdb:sync\` fills from IMDb's daily datasets.
...
- DB client + schema tables: \`import { db, imdbTitle } from "@packages/db"\`
